...
print "Graduation rate of the class: {:.2f}%".format(grad_rate * 100)  # output is intended to be in precents
...
# modified versions, to return values
...
def predict_labels(clf, features, target):
    ...
    # sys.stdout.write('| prt %.4f |' % (end - start))
...
def train_predict(clf, X_train, y_train, X_test, y_test):
    ...
    tr_time = train_classifier(clf, X_train, y_train)

    # Print the results of prediction for both training and testing
    # print "F1 score for training set: {:.4f}.".format(predict_labels(clf, X_train, y_train))
    # print "F1 score for test set: {:.4f}.".format(predict_labels(clf, X_test, y_test))
    f1_train, _ = predict_labels(clf, X_train, y_train)
    f1_test, pr_time = predict_labels(clf, X_test, y_test)
    sys.stdout.write('| %d | %.6f | %.6f | %.4f | %.4f |\n' % (len(X_train), tr_time, pr_time, f1_train, f1_test))
...
for idx, clf in enumerate([clf_A, clf_B, clf_C], start=1):
    ...
    sys.stdout.write('| Training Set Size | Training Time | Prediction Time (test) ')
    sys.stdout.write('| F1 Score (train) | F1 Score (test) |\n')
    sys.stdout.write('| :---------------: | :---------------------: | :--------------------: ')
    sys.stdout.write('| :--------------: | :-------------: |\n')
    for train_set_size in [100, 200, 300]:
